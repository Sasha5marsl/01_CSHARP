...
{
    for (int i = 2; i < num; i++) 
    // в этом цикле при помощи переменной i будем делить входящее из массива число 
    // на все числf до входящего числа не включительно, 
    // чтобы проверить делиться ли оно на что-то еще , кроме себя самого и единицы. Но в диапазон не включим 1 и само число. 
    // Поэтому, в случае, если входящее число делиться нацело хотя бы на одно число из заданного промежутка,
    // то программа ничего не вернет. А вернет только те, что выходят при верном значении функции. 
    //Например, 9 начнем делить с двойки, затем увеличивая в каждой итерации i на один, 
    // будем далее делать на 3, 4 и т.д. Получим, что 9 делится нацело (т.е. без остатка) еще и на 3. Число не подходит нам.
    {
